...
HWND checkbox, static1, Render_arrow, sens, buttons1, buttons2, buttons3, set_buttons, edge[3], pedals, zone_power[2][3], arrow[2];
...
VOID Set_Settings()
{
	...
	_itow_s(settings->sens, buffer, 5, 10);
	Edit_SetText(sens, buffer);
...
VOID WM_Keydown(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	if (keyflag == TRUE)
	{
		...
		if (psettings == NULL) { PrintError(L"Null psettings.\n"); return; }
		if (Index >= 5)
		...
		case EN_CHANGE:
		{
			if (create1)
			{
				...
				if ((HWND)lParam == sens)
				{
					WCHAR buffer_sens[4];
					buffer_sens[3] = '\0';
					Edit_GetText(sens, buffer_sens, 4);
					settings->sens = _wtoi(buffer_sens);
				}
				else
				{
...
{	
	/*if (sens = CreateWindowEx(WS_EX_OVERLAPPEDWINDOW, TRACKBAR_CLASS, L"", WM_HSCROLL | WS_VISIBLE | WS_CHILD | TBS_HORZ | TBS_FIXEDLENGTH, 35, 30, 220, 25, hWnd, NULL, NULL, NULL))
	{
		SendMessage(sens, TBM_SETTHUMBLENGTH, (WPARAM)20, (LPARAM)0);
	}
	...
	static1 = CreateWindow(WC_STATIC, L"Sensitivity->", WS_VISIBLE | WS_CHILD | ES_NUMBER | ES_CENTER, 70, 30, 90, 20, hWnd, NULL, NULL, NULL);
	if (sens = CreateWindow(WC_EDIT, L"", WS_VISIBLE | WS_CHILD | ES_NUMBER | ES_CENTER, 170, 30, 40, 20, hWnd, NULL, NULL, NULL))
	{
		SendMessage(sens, EM_LIMITTEXT, 4, 0);
	}
	...
	if (buttons2 = CreateWindow(WC_COMBOBOX, L"", WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | CBS_AUTOHSCROLL | WS_HSCROLL | WS_VSCROLL, 155, 70, 100, 100, hWnd, NULL, NULL, NULL))
	{
		TCHAR Buttons2[4][13] =
		{
			TEXT("None") ,TEXT("CursorTop"), TEXT("CursorBottom"), TEXT("CursorCenter")
		};
		TCHAR C[13];
...
VOID WM_Create_Render(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	Render_arrow = CreateWindowEx(0, WC_STATIC, L"", SS_CENTERIMAGE | SS_BITMAP |  WS_CHILD | WS_VISIBLE, (INT)(res.x/2.5), 0, (INT)(res.x/30), (INT)(res.y/1.2), hWnd, NULL, NULL, NULL);
